...
//        But then, shouldn't that be done in OckStation instead ? Or not ?
...
struct _OckPlaylistPrivate {
	gchar             *uri;
	OckPlaylistFormat format;
	GSList           *streams;
};

typedef struct _OckPlaylistPrivate OckPlaylistPrivate;

struct _OckPlaylist {
	/* Parent instance structure */
	GObject parent_instance;
	/* Private data */
	OckPlaylistPrivate *priv;
};

G_DEFINE_TYPE_WITH_PRIVATE(OckPlaylist, ock_playlist, G_TYPE_OBJECT)
...
on_message_completed(SoupSession *session,
                     ...
                     OckPlaylist *self)
{
	OckPlaylistPrivate *priv = self->priv;
	...
	switch (priv->format) {
	case OCK_PLAYLIST_FORMAT_M3U:
		parser = parse_playlist_m3u;
		break;
	case OCK_PLAYLIST_FORMAT_PLS:
		parser = parse_playlist_pls;
		break;
	case OCK_PLAYLIST_FORMAT_ASX:
		parser = parse_playlist_asx;
		break;
	case OCK_PLAYLIST_FORMAT_XSPF:
...
const gchar *
ock_playlist_get_uri(OckPlaylist *self)
{
	return self->priv->uri;
}

void
ock_playlist_set_uri(OckPlaylist *self, const gchar *uri)
{
	OckPlaylistPrivate *priv = self->priv;
	...
	/* Set format */
	priv->format = ock_playlist_get_format(uri);
...
GSList *
ock_playlist_get_stream_list(OckPlaylist *self)
{
	return self->priv->streams;
}

static void
ock_playlist_get_property(GObject    *object,
                          guint       property_id,
                          GValue     *value,
                          GParamSpec *pspec)
{
	OckPlaylist *self = OCK_PLAYLIST(object);

	TRACE_GET_PROPERTY(object, property_id, value, pspec);

	switch (property_id) {
	case PROP_URI:
		g_value_set_string(value, ock_playlist_get_uri(self));
		break;
	case PROP_STREAM_LIST:
		g_value_set_pointer(value, ock_playlist_get_stream_list(self));
		break;
	default:
...
static void
ock_playlist_set_property(GObject      *object,
                          guint         property_id,
                          const GValue *value,
                          GParamSpec   *pspec)
{
	...
	switch (property_id) {
	case PROP_URI:
		ock_playlist_set_uri(self, g_value_get_string(value));
		break;
	default:
		G_OBJECT_WARN_INVALID_PROPERTY_ID(object, property_id, pspec);
		break;
...
void
ock_playlist_download(OckPlaylist *self)
{
	OckPlaylistPrivate *priv = self->priv;
	SoupSession *session;
	SoupMessage *msg;
...
OckPlaylist *
ock_playlist_new(const gchar *uri)
{
	return g_object_new(OCK_TYPE_PLAYLIST, "uri", uri, NULL);
}
...
static void
ock_playlist_finalize(GObject *object)
{
	OckPlaylistPrivate *priv = OCK_PLAYLIST(object)->priv;
	...
	/* Chain up */
	G_OBJECT_CHAINUP_FINALIZE(ock_playlist, object);
}

static void
ock_playlist_init(OckPlaylist *self)
{
	TRACE("%p", self);

	/* Initialize private pointer */
	self->priv = ock_playlist_get_instance_private(self);
}

static void
ock_playlist_class_init(OckPlaylistClass *class)
{
	...
	object_class->finalize = ock_playlist_finalize;

	/* Properties */
	object_class->get_property = ock_playlist_get_property;
	object_class->set_property = ock_playlist_set_property;
...
OckPlaylistFormat
ock_playlist_get_format(const gchar *uri)
{
	char *ptr;
	...
	if (!g_ascii_strcasecmp(ptr, "m3u") ||
		...
		return OCK_PLAYLIST_FORMAT_ASX;
	else if (!g_ascii_strcasecmp(ptr, "xspf"))
		return OCK_PLAYLIST_FORMAT_XSPF;
